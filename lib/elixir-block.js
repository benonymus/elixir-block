...
export default {
    ...
    this.subscriptions.add(
      atom.workspace.observeActiveTextEditor(function (editor) {
        ...
        if (self.grammar.name == "Elixir") {
          this.editorSubscription = self.editor.onDidChangeCursorPosition(
            function () {
  ...
  onMatchableKeyword() {
    ...
    if (
      this.matchableDefiningToken(potentialMatchables) ||
      this.matchableFnToken(potentialMatchables) ||
      this.matchableDoToken(potentialMatchables)
    ) {
  ...
  controlToken(token) {
    ...
    return ["meta.module.elixir", "keyword.control.elixir"].includes(
      token.scopes[1]
    );
  },
  ...
  matchableDoToken(tokens) {
    return tokens.length == 1 && this.doToken(tokens[0]);
  },
  ...
  highlightMatchingDefineToken(currentLine) {
    ...
    pushStackFunc = (potentialMatchables) =>
      this.matchableEndToken(potentialMatchables);
    nextLineFunc = (lineNum) => lineNum - 1;

    this.highlightMatchingToken(
      currentLine,
      loopConditionFunc,
      matchableExistsFunc,
      pushStackFunc,
  ...
  highlightMatchingToken(
    ...
    for (
      lineNum = currentLine;
      loopConditionFunc(lineNum);
      lineNum = nextLineFunc(lineNum)
    ) {
      text = this.editor.lineTextForBufferRow(lineNum);
      tokens = this.grammar.tokenizeLine(text).tokens;
      potentialMatchables = tokens.filter((token) => this.controlToken(token));

      if (pushStackFunc(potentialMatchables)) {
